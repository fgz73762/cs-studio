...
public class RunModeService {
	...
	public enum TargetWindow {
		NEW_WINDOW, EDM_WINDOW, SAME_WINDOW, RUN_WINDOW;
	}
	...
	public static Rectangle getWindowBounds(IPath path) {
	...
	//
	//
	// public void replaceActiveEditorContent(IRunnerInput input) throws
	// PartInitException{
	...
	public static void replaceOPIRuntimeContent(final IOPIRuntime opiRuntime,
			final IEditorInput input) throws PartInitException {
		opiRuntime.setOPIInput(input);
	}

	/**
	 * Run an OPI file with necessary parameters. This function should be called
	 * when open an OPI from another OPI.
	 * 
	 * @param path
	 * @param targetWindow
	 ...
	 * @param macrosInput
	 */
	public void runOPI(IPath path, TargetWindow targetWindow,
			DisplayOpenManager displayOpenManager, MacrosInput macrosInput) {
	 ...
	 * 
	 * @param path
	 *            the file to be ran. If displayModel is not null, this will be
	 *            ignored.
	 * @param displayModel
	 *            the display model to be ran. null for file input only.
	 * @param displayOpenManager
	 *            the manager help to manage the opened displays. null if the
	 *            OPI is not replacing the current active display.
	 */
	public void runOPI(final IPath path, final TargetWindow target,
			final DisplayOpenManager displayOpenManager,
			final MacrosInput macrosInput, final Rectangle windowBounds) {
		final RunnerInput runnerInput = new RunnerInput(path,
				displayOpenManager, macrosInput);
			...
			public void run() {
				...
				case RUN_WINDOW:
					if (runWorkbenchWindow == null) {
						...
						for (IEditorReference editor : runWorkbenchWindow
								.getActivePage().getEditorReferences()) {
							try {
								if (editor.getEditorInput().equals(runnerInput))
									editor.getPage().closeEditor(
				...
				case SAME_WINDOW:
				default:
					targetWindow = PlatformUI.getWorkbench()
	...
	public static void runOPIInView(final IPath path,
			final DisplayOpenManager displayOpenManager,
			final MacrosInput macrosInput, final Position position) {
				...
				try {
					...
					if (position != Position.DETACHED
							...
							&& !(page.getPerspective().getId()
									.equals(OPIRunnerPerspective.ID))) {
						int openCode = 0;
						if (!OPIBuilderPlugin.isRAP()
								&& PreferencesHelper
										.isShowOpiRuntimePerspectiveDialog()) {
							...
							if (!dialog.isShowThisDialogAgain())
								PreferencesHelper
										.setShowOpiRuntimePerspectiveDialog(false);
						}
					...
					ErrorHandlerUtil.handleError(
							NLS.bind("Failed to run OPI {1} in view.", path), e);
				}
			}
		});
	}
	...
	private IWorkbenchWindow createNewWindow(Rectangle windowBounds) {
		...
		try {
			newWindow = PlatformUI.getWorkbench().openWorkbenchWindow(
					OPIRunnerPerspective.ID, null); //$NON-NLS-1$
			if (windowBounds != null) {
				if (windowBounds.x >= 0 && windowBounds.y > 1)
					newWindow.getShell().setLocation(windowBounds.x,
							windowBounds.y);
				newWindow.getShell().setSize(windowBounds.width + 45,
						windowBounds.height + 165);
			}

		} catch (WorkbenchException e) {
			OPIBuilderPlugin.getLogger().log(Level.WARNING,
					"Failed to open new window", e);
		}
		return newWindow;
	}
