...
public class EditItemsCommand implements IUndoableCommand {
	 ...
	 * 
	 * @param shell
	 *            Shell for error message dialogs.
	 * @param operations_manager
	 *            Operations manager which manages undo/redo commands.
	 * @param items
	 *            Array of ModelItem instances subjected to editing.
	 * @param result
	 *            Result instance returned by EditItemsDialog.
	 */
	public EditItemsCommand(final OperationsManager operations_manager,
			final ModelItem[] items, final EditItemsDialog.Result result) {
		this.items = items;
		this.result = result;
	...
	public void undo() {
		for (int i = 0; i < items.length; i++) {
			...
			if (item instanceof PVItem) {
				try {
					((PVItem) item).setScanPeriod(((PVItem) oldItem)
							.getScanPeriod());
				} catch (Exception e) {
					e.printStackTrace();
				}
