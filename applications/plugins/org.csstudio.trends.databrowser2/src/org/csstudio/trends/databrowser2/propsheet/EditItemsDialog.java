...
public class EditItemsDialog extends Dialog {
	...
	public class Result {
		...
		public boolean appliedVisible() {
			return applyVisible;
		}

		public boolean appliedItem() {
			return applyItem;
		}

		public boolean appliedDisplayName() {
			return applyDisplayName;
		}

		public boolean appliedColor() {
			return applyColor;
		}

		public boolean appliedScan() {
			return applyScan;
		}

		public boolean appliedBufferSize() {
			return applyBufferSize;
		}

		public boolean appliedWidth() {
			return applyWidth;
		}

		public boolean appliedAxis() {
			return applyAxis;
		}

		public boolean appliedTraceType() {
			return applyTraceType;
		}

		public boolean appliedRequest() {
			return applyRequest;
		}

		public boolean appliedIndex() {
			return applyIndex;
		}

		public boolean appliedErrorType() {
			return applyErrorType;
		}
	}

	/**
	 * Implementation of VerifyListener to check if the entered text is a
	 * numerical value.
	 * 
	 * @author Takashi Nakamoto
	 */
	private class NumericalVerifyListener implements VerifyListener {
		 ...
		 * 
		 * @param text
		 *            SWT Text widget to be verified.
		 * @param isDouble
		 *            True to check if the entered text is a double value. Flase
		 *            to check if is an integer value.
		 */
	...
	protected Control createDialogArea(Composite parent) {
		...
		for (ModelItem item : items) {
			if (item instanceof PVItem) {
				textScan.setText(Double.toString(((PVItem) item)
						.getScanPeriod()));
				enableScan = true;
				break;
			}
		...
		for (ModelItem item : items) {
			if (item instanceof PVItem) {
				textBufferSize.setText(Integer.toString(((PVItem) item)
						.getLiveCapacity()));
		...
		textBufferSize.addVerifyListener(new NumericalVerifyListener(
				textBufferSize, false));
		...
		textWidth.addVerifyListener(new NumericalVerifyListener(textWidth,
				false));
		...
		textIndex.addVerifyListener(new NumericalVerifyListener(textIndex,
				false));
	...
	protected void buttonPressed(int buttonId) {
		...
		if (IDialogConstants.OK_ID == buttonId) {
			...
			result.traceType = TraceType.values()[cmbTraceType
					.getSelectionIndex()];
			result.request = RequestType.values()[cmbRequest
					.getSelectionIndex()];
			try {
			...
			result.errorType = ErrorType.values()[cmbErrorType
					.getSelectionIndex()];
		}
	...
	/**
	 * Get the result of this dialog. This method returns null if the dialog is
	 * not closed yet, or if the dialog is closed with "Cancel" button.
	 * 
